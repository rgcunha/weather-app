...
export const fetchCurrentWeather = (cityNames) => {
  ...
  cityNames.forEach((cityName) => {
    let url = `${API_URL}/weather?q=${cityName}&APPID=${API_KEY}`;
    promises.push(axios.get(url));
  });
  return axios.all(promises)
    .then((response) => response.map((cityResponse) => cityResponse.data))
}

export const fetchCurrentWeatherSync = (cityNames) => {
  return samples.filter((city) => cityNames.indexOf(city.name) > -1);
}
